...
This is to make sure the random elements in models start from the same point every time. This will make our results reproducible.
...
#### Semibalanus response curve plots
...
SB_diff_plot_future <- ggplot() +
  ...
  labs(fill = NULL, title = "Semibalanus balanoides") +
  coord_sf(
    xlim = c(-8, 3),
    ylim = c(49.7, 61),
    expand = FALSE
...
CM_diff_plot_future <- ggplot() +
  ...
  labs(fill = NULL, title = "Chthamalus montagui") +
  coord_sf(
    xlim = c(-8, 3),
    ylim = c(49.7, 61),
    expand = FALSE
...
# Make a dataset with start (current) and end (future) positions for each lat_bin & species
arrow_data <- lat_bin_summary %>%
  ...
  rename(current = suitability_current,
         future = suitability_future)

arrow_data$future = arrow_data$future - 0.01
...
Lat_bin_line_plot <- ggplot(lat_bin_summary,
  ...
  geom_pointrange(aes(ymin = mean_suit - se, ymax = mean_suit + se),
                  size = 1) +
...
# Save the plot
ggsave(Lat_bin_line_plot, filename = "Figures/Lat_bin_line_plot.jpeg", width = 8, height = 5, dpi = 300)
```
...
# Save the plot
ggsave(forecast_plot, filename = "Figures/forecast_plot.jpeg", width =10, height = 8, dpi = 300)
```
