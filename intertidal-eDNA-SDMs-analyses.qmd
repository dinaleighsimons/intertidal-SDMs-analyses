...
### Create final data sets for model
...
semibalanus_balanoides_obs_visual$source <- "MarClim"
final_visual_semibalanus <- rbind(semibalanus_GBIF_clean_reduced, semibalanus_balanoides_obs_visual)
...
We now have two data sets for each species, one with MarClim + GBIF visual observations and one with all visual + eDNA.

```{r}
#| label: view-final-datasets

final_visual_semibalanus
final_combined_semibalanus
```
...
names(enviro_vars_current) <- c("ocean_temp_2000", "ocean_temp_2020", "air_temp_2000", "air_temp_2020", "ph_2000", "ph_2020", "coastline_binary", "rocky_substrate")
...
presvals_semibalanus_corrected <- get_nearest_coastline(
  enviro_vars_current,
  final_visual_semibalanus_reduced,
  terrain)
