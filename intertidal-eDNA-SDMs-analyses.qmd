...
#backgr <- spatSample(enviro_vars_current_rast,
                     #500,
                     #"random",
                     #as.points = TRUE,
                     #na.rm = TRUE)
...
#m1 <- glm(pb ~ ocean_temp_2020 + ph_2020 + rocky_substrate, data=sdmdata)
#class(m1)
...
#bc <- envelope(presvals_semibalanus_corrected[,c("ocean_temp_2020", "ph_2020")])
#bc

#pr <- partialResponse(bc, presvals_semibalanus_corrected, "ocean_temp_2020")
#plot(pr[[1]], type="l")
```

```{r}

#names(enviro_vars_current_rast)
#p <- predict(enviro_vars_current_rast, m1)

# 'coastline' is a binary raster: 1 = coastline, NA or 0 = non-coast
#prediction_masked <- mask(p, terrain)
#plot(prediction_masked) # obviously this need to be restircted to rocky areas
```

```{r}

# Convert to points
#points_df <- as.data.frame(prediction_masked, xy = TRUE, na.rm = TRUE)
#colnames(points_df)[3] <- "probability"

#ggplot(points_df, aes(x = x, y = y, color = probability)) +
  #geom_point(size = 0.6) +
  #scale_color_viridis_c(name = "Habitat Suitability") +
  #theme_minimal() +
  #coord_fixed()
```
...
```{r}
#| label: set-maxent-path

# Set absolute or relative path to maxent.jar
#maxent_jar_path <- file.path(getwd(), "Modelling_files", "maxent", "maxent.jar")
#file.exists(maxent_jar_path)
#maxent()

# Check it exists
#if (!file.exists(maxent_jar_path)) stop("maxent.jar not found at specified path")

# Tell dismo where MaxEnt is
#options(dismo.maxent = maxent_jar_path)
...
get_evaluations(myBiomodModelOut_semibalanus_visual)
get_variables_importance(myBiomodModelOut_semibalanus_visual)
