...
chthamalus_map_legend <- ggplot() +
  ...
  coord_sf(xlim = c(-9, 3), ylim = c(49.7, 61)) +
  theme_minimal() +
  labs(x = "Longitude", y = "Latitude", color = "Data source") +
  scale_color_manual(values = c(
    "GBIF" = GBIF_colour,
...
It's time to find environmental variables of interest. We have a few options here - let's explore them.

OCLE (<https://ocle.ihcantabria.com/>) provides meteo-oceanographic data for UK coastal communities, some of which are projected under future climate change scenarios. To match the resolution of the projected estimates, we are going to download annual mean for each variable. Summer mean is available for historic periods, but not for projected periods. Decided not to use this one.
...
2009 wave fetch data is downloaded from <https://www.sams.ac.uk/people/researchers/burrows-professor-michael/>.
...
fetch_small <- aggregate(fetch, fact = 5, fun = mean)
...
fetch_df <- as.data.frame(fetch_small, xy = TRUE) %>% na.omit()
...
# Assuming 'fetch' is your SpatRaster object
hist(fetch_small, main = "Wave Exposure Distribution", xlab = "Exposure Index", col = "lightblue")
...
Provided an updated data set in 2012 accessible here - <https://figshare.com/articles/dataset/Wave_fetch_GIS_layers_for_the_UK_and_Ireland_at_200m_scale/12029682?file=22107477>.
...
#### Cross validation 1 (Semibalanus - visual)

```{r}
#| label: cross-val-semibalanus-visual

set.seed(seed.val.set)
cv.sv <- bm_CrossValidation(bm.format = myBiomodData_semibalanus_visual,
                            strategy = 'random',
                            nb.rep = 2,
                            perc = 0.7)

print(cv.sv)
summary(myBiomodData_semibalanus_visual, calib.lines = cv.sv)

# Save plot to PNG
png("Figures/myBiomodData_validation_semibalanus_visual.png", 
    width = 1500, height = 2500, res = 300)  # size in pixels, resolution in DPI
plot(myBiomodData_semibalanus_visual, calib.lines = cv.sv)
dev.off()
```

#### Cross validation 2 (Semibalanus - combined)

```{r}
#| label: cross-val-semibalanus-combined

set.seed(seed.val.set)
cv.sc <- bm_CrossValidation(bm.format = myBiomodData_semibalanus_combined,
                            strategy = 'random',
                            nb.rep = 2,
                            perc = 0.7)

print(cv.sc)
summary(myBiomodData_semibalanus_combined, calib.lines = cv.sc)

# Save plot to PNG
png("Figures/myBiomodData_validation_semibalanus_combined.png", 
    width = 1500, height = 2500, res = 300)  # size in pixels, resolution in DPI
plot(myBiomodData_semibalanus_combined, calib.lines = cv.sc)
dev.off()
```

#### Cross validation 3 (Chthamalus - visual)

```{r}
#| label: cross-val-chthamalus-visual

set.seed(seed.val.set)
cv.cv <- bm_CrossValidation(bm.format = myBiomodData_chthamalus_visual,
                            strategy = 'random',
                            nb.rep = 2,
                            perc = 0.7)

print(cv.cv)
summary(myBiomodData_chthamalus_visual, calib.lines = cv.cv)

# Save plot to PNG
png("Figures/myBiomodData_validation_chthamalus_visual.png", 
    width = 1500, height = 2500, res = 300)  # size in pixels, resolution in DPI
plot(myBiomodData_chthamalus_visual, calib.lines = cv.cv)
dev.off()
```

#### Cross validation 4 (Chthamalus - combined)

```{r}
#| label: cross-val-chthamalus-combined

set.seed(seed.val.set)
cv.cc <- bm_CrossValidation(bm.format = myBiomodData_chthamalus_combined,
                            strategy = 'random',
                            nb.rep = 2,
                            perc = 0.7)

print(cv.cc)
summary(myBiomodData_chthamalus_combined, calib.lines = cv.cc)

# Save plot to PNG
png("Figures/myBiomodData_validation_chthamalus_combined.png", 
    width = 1500, height = 2500, res = 300)  # size in pixels, resolution in DPI
plot(myBiomodData_chthamalus_combined, calib.lines = cv.cc)
dev.off()
```
...
# Set paths
table_path <- "Data/Processed_Data/Semi.bal/Individual-Visual/"
figure_path <- "Figures/Semi.bal/Individual-Visual/"
...
eval_scores <- get_evaluations(myBiomodModelOut_semibalanus_visual)
var_imp <- get_variables_importance(myBiomodModelOut_semibalanus_visual)
...
# calibration means
eval_calib <- bm_PlotEvalMean(bm.out = myBiomodModelOut_semibalanus_visual, dataset = "calibration")
...
# evaluation means
eval_eval <- bm_PlotEvalMean(bm.out = myBiomodModelOut_semibalanus_visual, dataset = "evaluation")
...
eval_boxplot_algo <- bm_PlotEvalBoxplot(bm.out = myBiomodModelOut_semibalanus_visual, group.by = c('algo', 'algo'))
...
eval_boxplot_algo_run <- bm_PlotEvalBoxplot(bm.out = myBiomodModelOut_semibalanus_visual, group.by = c('algo', 'run'))
...
val_importance_boxplot <- bm_PlotVarImpBoxplot(bm.out = myBiomodModelOut_semibalanus_visual, group.by = c('expl.var', 'algo', 'algo'))
...
val_importance_boxplot_run <- bm_PlotVarImpBoxplot(bm.out = myBiomodModelOut_semibalanus_visual, group.by = c('expl.var', 'algo', 'run'))
...
# Response curves
models_to_plot <- get_built_models(myBiomodModelOut_semibalanus_visual)[c(1:3, 12:14)]

# median
response_median <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_semibalanus_visual, 
...
# min
response_min<-bm_PlotResponseCurves(bm.out = myBiomodModelOut_semibalanus_visual, 
                      models.chosen = models_to_plot,
                      fixed.var = 'min')
...
# heat map - median
response_heatmap_median <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_semibalanus_visual, 
...
# Set paths
table_path <- "Data/Processed_Data/Semi.bal/Ensemble-Visual/"
figure_path <- "Figures/Semi.bal/Ensemble-Visual/"
...
# Get evaluation scores & variable importance tables
eval_scores <- get_evaluations(myBiomodEM_semibalanus_visual)
var_imp <- get_variables_importance(myBiomodEM_semibalanus_visual)

write.csv(eval_scores, paste0(table_path, "Evaluations_All.csv"), row.names = TRUE)
write.csv(var_imp, paste0(table_path, "VariablesImportance_All.csv"), row.names = TRUE)

# calibration means
eval_calib <- bm_PlotEvalMean(bm.out = myBiomodEM_semibalanus_visual, group.by = 'full.name')
...
# evaluation means
eval_eval <- bm_PlotEvalMean(bm.out = myBiomodEM_semibalanus_visual, dataset = "evaluation")
...
# calibration boxplots - overall 
# table produced here same as above, already saved
eval_boxplot_algo <- bm_PlotEvalBoxplot(bm.out = myBiomodEM_semibalanus_visual, group.by = c('full.name', 'full.name'))
...
# Variable importance boxplots - overall 
# figures (table produced here same as above, already saved
val_importance_boxplot <- bm_PlotVarImpBoxplot(bm.out = myBiomodEM_semibalanus_visual, c('expl.var', 'full.name', 'full.name'))
plot <- val_importance_boxplot$plot + theme_classic() + labs(fill = "Model")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

val_importance_boxplot_merged <- bm_PlotVarImpBoxplot(bm.out = myBiomodEM_semibalanus_visual, group.by = c('expl.var', 'algo', 'merged.by.run'))
plot <- val_importance_boxplot_merged$plot + theme_classic() + labs(fill = "Model")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

# Response curves
models_to_plot <- get_built_models(myBiomodEM_semibalanus_visual)[c(1, 6, 7)]

# median
response_median <- bm_PlotResponseCurves(bm.out = myBiomodEM_semibalanus_visual, 
                      models.chosen = models_to_plot,
                      fixed.var = 'median')
tab <- response_median$tab
plot <- response_median$plot + theme_classic() + labs(colour = "Prediction Name")
...
# min
response_min<-bm_PlotResponseCurves(bm.out = myBiomodEM_semibalanus_visual, 
                      models.chosen = models_to_plot,
                      fixed.var = 'min')
tab <- response_min$tab
plot <- response_min$plot + theme_classic() + labs(colour = "Prediction Name")
...
# heat map - median
response_heatmap_median <- bm_PlotResponseCurves(bm.out = myBiomodEM_semibalanus_visual, 
                      models.chosen = get_built_models(myBiomodEM_semibalanus_visual)[3],
                      fixed.var = 'median',
                      do.bivariate = TRUE)
plot <- response_heatmap_median$plot + theme_classic()
...
# Set paths
table_path <- "Data/Processed_Data/Semi.bal/Individual-Combined/"
figure_path <- "Figures/Semi.bal/Individual-Combined/"
...
# Get evaluation scores & variable importance tables
eval_scores <- get_evaluations(myBiomodModelOut_semibalanus_combined)
var_imp <- get_variables_importance(myBiomodModelOut_semibalanus_combined)

write.csv(eval_scores, paste0(table_path, "Evaluations_All.csv"), row.names = TRUE)
write.csv(var_imp, paste0(table_path, "VariablesImportance_All.csv"), row.names = TRUE)

# calibration means
eval_calib <- bm_PlotEvalMean(bm.out = myBiomodModelOut_semibalanus_combined, dataset = "calibration")
...
# evaluation means
eval_eval <- bm_PlotEvalMean(bm.out = myBiomodModelOut_semibalanus_combined, dataset = "evaluation")
...
# calibration boxplots - overall 
# table produced here same as above, already saved
eval_boxplot_algo <- bm_PlotEvalBoxplot(bm.out = myBiomodModelOut_semibalanus_combined, group.by = c('algo', 'algo'))
plot <- eval_boxplot_algo$plot + theme_classic() + labs(fill = "Algorithm",
                                                y = "Calibration")
ggsave(plot, file = paste0(figure_path, "EvalMean_boxplot.png"), width = 7, height = 5)

# calibration boxplots - by run
eval_boxplot_algo_run <- bm_PlotEvalBoxplot(bm.out = myBiomodModelOut_semibalanus_combined, group.by = c('algo', 'run'))
plot <- eval_boxplot_algo_run$plot + theme_classic() + labs(fill = "Algorithm",
                                                y = "Calibration")
ggsave(plot, file = paste0(figure_path, "EvalMean_boxplot_run.png"), width = 8, height = 6)
...
# Variable importance boxplots - overall 
# figures (table produced here same as above, already saved
val_importance_boxplot <- bm_PlotVarImpBoxplot(bm.out = myBiomodModelOut_semibalanus_combined, group.by = c('expl.var', 'algo', 'algo'))
plot <- val_importance_boxplot$plot + theme_classic() + labs(fill = "Algorithm")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

# Variable importance boxplots - by run
val_importance_boxplot_run <- bm_PlotVarImpBoxplot(bm.out = myBiomodModelOut_semibalanus_combined, group.by = c('expl.var', 'algo', 'run'))
plot <- val_importance_boxplot_run$plot + theme_classic() + labs(fill = "Model")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo_Run.png"), width = 8, height = 6)

# Response curves
models_to_plot <- get_built_models(myBiomodModelOut_semibalanus_combined)[c(1:3, 12:14)]

# median
response_median <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_semibalanus_combined, 
                      models.chosen = models_to_plot,
                      fixed.var = 'median')
tab <- response_median$tab
plot <- response_median$plot + theme_classic() + labs(colour = "Prediction Name")
...
# min
response_min<-bm_PlotResponseCurves(bm.out = myBiomodModelOut_semibalanus_combined, 
                      models.chosen = models_to_plot,
                      fixed.var = 'min')
tab <- response_min$tab
plot <- response_min$plot + theme_classic() + labs(colour = "Prediction Name")
...
# heat map - median
response_heatmap_median <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_semibalanus_combined, 
...
# Set paths
table_path <- "Data/Processed_Data/Semi.bal/Ensemble-Combined/"
figure_path <- "Figures/Semi.bal/Ensemble-Combined/"
...
# Get evaluation scores & variable importance tables
eval_scores <- get_evaluations(myBiomodEM_semibalanus_combined)
var_imp <- get_variables_importance(myBiomodEM_semibalanus_combined)

write.csv(eval_scores, paste0(table_path, "Evaluations_All.csv"), row.names = TRUE)
write.csv(var_imp, paste0(table_path, "VariablesImportance_All.csv"), row.names = TRUE)

# calibration means
eval_calib <- bm_PlotEvalMean(bm.out = myBiomodEM_semibalanus_combined, group.by = 'full.name')
...
# evaluation means
eval_eval <- bm_PlotEvalMean(bm.out = myBiomodEM_semibalanus_combined, dataset = "evaluation")
...
# calibration boxplots - overall 
# table produced here same as above, already saved
eval_boxplot_algo <- bm_PlotEvalBoxplot(bm.out = myBiomodEM_semibalanus_combined, group.by = c('full.name', 'full.name'))
...
# Variable importance boxplots - overall 
# figures (table produced here same as above, already saved
val_importance_boxplot <- bm_PlotVarImpBoxplot(bm.out = myBiomodEM_semibalanus_combined, c('expl.var', 'full.name', 'full.name'))
plot <- val_importance_boxplot$plot + theme_classic() + labs(fill = "Model")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

val_importance_boxplot_merged <- bm_PlotVarImpBoxplot(bm.out = myBiomodEM_semibalanus_combined, group.by = c('expl.var', 'algo', 'merged.by.run'))
plot <- val_importance_boxplot_merged$plot + theme_classic() + labs(fill = "Model")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

# Response curves
models_to_plot <- models_to_plot <- get_built_models(myBiomodEM_semibalanus_combined)[c(1, 6, 7)]

# median
response_median <- bm_PlotResponseCurves(bm.out = myBiomodEM_semibalanus_combined, 
                      models.chosen = models_to_plot,
                      fixed.var = 'median')
tab <- response_median$tab
plot <- response_median$plot + theme_classic() + labs(colour = "Prediction Name")
...
# min
response_min<-bm_PlotResponseCurves(bm.out = myBiomodEM_semibalanus_combined, 
                      models.chosen = models_to_plot,
                      fixed.var = 'min')
tab <- response_min$tab
plot <- response_min$plot + theme_classic() + labs(colour = "Prediction Name")
...
# heat map - median
response_heatmap_median <- bm_PlotResponseCurves(bm.out = myBiomodEM_semibalanus_combined, 
...
SB_assessment_plot <- cowplot::plot_grid(
  ...
  ncol = 2, 
  labels = c("a) Ensemble (GBIF-only)", 
             "b) Ensemble (GBIF-eDNA)", 
             "c) Algorithms (Calibration, GBIF-only)", 
             "d) Algorithms (Calibration, GBIF-eDNA)",
             "e) Algorithms (Evaluation, GBIF-only)",
...
# Set paths
table_path <- "Data/Processed_Data/Chtham.mont/Individual-Visual/"
figure_path <- "Figures/Chtham.mont/Individual-Visual/"
...
# Get evaluation scores & variable importance tables
eval_scores <- get_evaluations(myBiomodModelOut_chthamalus_visual)
var_imp <- get_variables_importance(myBiomodModelOut_chthamalus_visual)

write.csv(eval_scores, paste0(table_path, "Evaluations_All.csv"), row.names = TRUE)
write.csv(var_imp, paste0(table_path, "VariablesImportance_All.csv"), row.names = TRUE)

# calibration means
eval_calib <- bm_PlotEvalMean(bm.out = myBiomodModelOut_chthamalus_visual, dataset = "calibration")
...
# evaluation means
eval_eval <- bm_PlotEvalMean(bm.out = myBiomodModelOut_chthamalus_visual, dataset = "evaluation")
...
# calibration boxplots - overall 
# table produced here same as above, already saved
eval_boxplot_algo <- bm_PlotEvalBoxplot(bm.out = myBiomodModelOut_chthamalus_visual, group.by = c('algo', 'algo'))
plot <- eval_boxplot_algo$plot + theme_classic() + labs(fill = "Algorithm",
                                                y = "Calibration")
ggsave(plot, file = paste0(figure_path, "EvalMean_boxplot.png"), width = 7, height = 5)

# calibration boxplots - by run
eval_boxplot_algo_run <- bm_PlotEvalBoxplot(bm.out = myBiomodModelOut_chthamalus_visual, group.by = c('algo', 'run'))
plot <- eval_boxplot_algo_run$plot + theme_classic() + labs(fill = "Algorithm",
                                                y = "Calibration")
ggsave(plot, file = paste0(figure_path, "EvalMean_boxplot_run.png"), width = 8, height = 6)
...
# Variable importance boxplots - overall 
# figures (table produced here same as above, already saved
val_importance_boxplot <- bm_PlotVarImpBoxplot(bm.out = myBiomodModelOut_chthamalus_visual, group.by = c('expl.var', 'algo', 'algo'))
plot <- val_importance_boxplot$plot + theme_classic() + labs(fill = "Algorithm")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

# Variable importance boxplots - by run
val_importance_boxplot_run <- bm_PlotVarImpBoxplot(bm.out = myBiomodModelOut_chthamalus_visual, group.by = c('expl.var', 'algo', 'run'))
plot <- val_importance_boxplot_run$plot + theme_classic() + labs(fill = "Algorithm")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo_Run.png"), width = 8, height = 6)

# Response curves
models_to_plot <- get_built_models(myBiomodModelOut_chthamalus_visual)[c(1:3, 12:14)]

# median
response_median <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_chthamalus_visual, 
                      models.chosen = models_to_plot,
                      fixed.var = 'median')
tab <- response_median$tab
plot <- response_median$plot + theme_classic() + labs(colour = "Prediction Name")
...
# min
response_min<-bm_PlotResponseCurves(bm.out = myBiomodModelOut_chthamalus_visual, 
                      models.chosen = models_to_plot,
                      fixed.var = 'min')
tab <- response_min$tab
plot <- response_min$plot + theme_classic() + labs(colour = "Prediction Name")
...
# heat map - median
response_heatmap_median <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_chthamalus_visual, 
...
# Set paths
table_path <- "Data/Processed_Data/Chtham.mont/Ensemble-Visual/"
figure_path <- "Figures/Chtham.mont/Ensemble-Visual/"
...
# Get evaluation scores & variable importance tables
eval_scores <- get_evaluations(myBiomodEM_chthamalus_visual)
var_imp <- get_variables_importance(myBiomodEM_chthamalus_visual)

write.csv(eval_scores, paste0(table_path, "Evaluations_All.csv"), row.names = TRUE)
write.csv(var_imp, paste0(table_path, "VariablesImportance_All.csv"), row.names = TRUE)

# calibration means
eval_calib <- bm_PlotEvalMean(bm.out = myBiomodEM_chthamalus_visual, group.by = 'full.name')
plot <- eval_calib$plot + theme_classic() + labs(colour = "Type")
tab <- eval_calib$tab
...
# evaluation means
eval_eval <- bm_PlotEvalMean(bm.out = myBiomodEM_chthamalus_visual, dataset = "evaluation")
...
# calibration boxplots - overall 
# table produced here same as above, already saved
eval_boxplot_algo <- bm_PlotEvalBoxplot(bm.out = myBiomodEM_chthamalus_visual, group.by = c('full.name', 'full.name'))
plot <- eval_boxplot_algo$plot + theme_classic() + labs(fill = "Type",
                                                y = "Calibration")
ggsave(plot, file = paste0(figure_path, "EvalMean_boxplot.png"), width = 7, height = 5)
...
# Variable importance boxplots - overall 
# figures (table produced here same as above, already saved
val_importance_boxplot <- bm_PlotVarImpBoxplot(bm.out = myBiomodEM_chthamalus_visual, c('expl.var', 'full.name', 'full.name'))
plot <- val_importance_boxplot$plot + theme_classic() + labs(fill = "Type")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

val_importance_boxplot_merged <- bm_PlotVarImpBoxplot(bm.out = myBiomodEM_chthamalus_visual, group.by = c('expl.var', 'algo', 'merged.by.run'))
plot <- val_importance_boxplot_merged$plot + theme_classic() + labs(fill = "Type")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

# Response curves
models_to_plot <- get_built_models(myBiomodEM_chthamalus_visual)[c(1, 6, 7)]

# median
response_median <- bm_PlotResponseCurves(bm.out = myBiomodEM_chthamalus_visual, 
                      models.chosen = models_to_plot,
                      fixed.var = 'median')
tab <- response_median$tab
plot <- response_median$plot + theme_classic() + labs(colour = "Prediction Name")
...
# min
response_min<-bm_PlotResponseCurves(bm.out = myBiomodEM_chthamalus_visual, 
                      models.chosen = models_to_plot,
                      fixed.var = 'min')
tab <- response_min$tab
plot <- response_min$plot + theme_classic() + labs(colour = "Prediction Name")
...
# heat map - median
response_heatmap_median <- bm_PlotResponseCurves(bm.out = myBiomodEM_chthamalus_visual, 
...
# Set paths
table_path <- "Data/Processed_Data/Chtham.mont/Individual-Combined/"
figure_path <- "Figures/Chtham.mont/Individual-Combined/"
...
# Get evaluation scores & variable importance tables
eval_scores <- get_evaluations(myBiomodModelOut_chthamalus_combined)
var_imp <- get_variables_importance(myBiomodModelOut_chthamalus_combined)

write.csv(eval_scores, paste0(table_path, "Evaluations_All.csv"), row.names = TRUE)
write.csv(var_imp, paste0(table_path, "VariablesImportance_All.csv"), row.names = TRUE)

# calibration means
eval_calib <- bm_PlotEvalMean(bm.out = myBiomodModelOut_chthamalus_combined, dataset = "calibration")
...
# evaluation means
eval_eval <- bm_PlotEvalMean(bm.out = myBiomodModelOut_chthamalus_combined, dataset = "evaluation")
...
# calibration boxplots - overall 
# table produced here same as above, already saved
eval_boxplot_algo <- bm_PlotEvalBoxplot(bm.out = myBiomodModelOut_chthamalus_combined, group.by = c('algo', 'algo'))
plot <- eval_boxplot_algo$plot + theme_classic() + labs(fill = "Algorithm",
                                                y = "Calibration")
ggsave(plot, file = paste0(figure_path, "EvalMean_boxplot.png"), width = 7, height = 5)

# calibration boxplots - by run
eval_boxplot_algo_run <- bm_PlotEvalBoxplot(bm.out = myBiomodModelOut_chthamalus_combined, group.by = c('algo', 'run'))
plot <- eval_boxplot_algo_run$plot + theme_classic() + labs(fill = "Algorithm",
                                                y = "Calibration")
ggsave(plot, file = paste0(figure_path, "EvalMean_boxplot_run.png"), width = 8, height = 6)
...
# Variable importance boxplots - overall 
# figures (table produced here same as above, already saved
val_importance_boxplot <- bm_PlotVarImpBoxplot(bm.out = myBiomodModelOut_chthamalus_combined, group.by = c('expl.var', 'algo', 'algo'))
plot <- val_importance_boxplot$plot + theme_classic() + labs(fill = "Algorithm")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

# Variable importance boxplots - by run
val_importance_boxplot_run <- bm_PlotVarImpBoxplot(bm.out = myBiomodModelOut_chthamalus_combined, group.by = c('expl.var', 'algo', 'run'))
plot <- val_importance_boxplot_run$plot + theme_classic() + labs(fill = "Algorithm")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo_Run.png"), width = 8, height = 6)

# Response curves
models_to_plot <- get_built_models(myBiomodModelOut_chthamalus_combined)[c(1:3, 12:14)]

# median
response_median <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_chthamalus_combined, 
                      models.chosen = models_to_plot,
                      fixed.var = 'median')
tab <- response_median$tab
plot <- response_median$plot + theme_classic() + labs(colour = "Prediction Name")
...
# min
response_min<-bm_PlotResponseCurves(bm.out = myBiomodModelOut_chthamalus_combined, 
                      models.chosen = models_to_plot,
                      fixed.var = 'min')
tab <- response_min$tab
plot <- response_min$plot + theme_classic() + labs(colour = "Prediction Name")
...
# heat map - median
response_heatmap_median <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_chthamalus_combined, 
...
# Set paths
table_path <- "Data/Processed_Data/Chtham.mont/Ensemble-Combined/"
figure_path <- "Figures/Chtham.mont/Ensemble-Combined/"
...
# Get evaluation scores & variable importance tables
eval_scores <- get_evaluations(myBiomodEM_chthamalus_combined)
var_imp <- get_variables_importance(myBiomodEM_chthamalus_combined)

write.csv(eval_scores, paste0(table_path, "Evaluations_All.csv"), row.names = TRUE)
write.csv(var_imp, paste0(table_path, "VariablesImportance_All.csv"), row.names = TRUE)

# calibration means
eval_calib <- bm_PlotEvalMean(bm.out = myBiomodEM_chthamalus_combined, group.by = 'full.name')
plot <- eval_calib$plot + theme_classic() + labs(colour = "Type")
tab <- eval_calib$tab
...
# evaluation means
eval_eval <- bm_PlotEvalMean(bm.out = myBiomodEM_chthamalus_combined, dataset = "evaluation")
...
# calibration boxplots - overall 
# table produced here same as above, already saved
eval_boxplot_algo <- bm_PlotEvalBoxplot(bm.out = myBiomodEM_chthamalus_combined, group.by = c('full.name', 'full.name'))
plot <- eval_boxplot_algo$plot + theme_classic() + labs(fill = "Type",
                                                y = "Calibration")
ggsave(plot, file = paste0(figure_path, "EvalMean_boxplot.png"), width = 10, height = 5)
...
# Variable importance boxplots - overall 
# figures (table produced here same as above, already saved
val_importance_boxplot <- bm_PlotVarImpBoxplot(bm.out = myBiomodEM_chthamalus_combined, c('expl.var', 'full.name', 'full.name'))
plot <- val_importance_boxplot$plot + theme_classic() + labs(fill = "Type")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

val_importance_boxplot_merged <- bm_PlotVarImpBoxplot(bm.out = myBiomodEM_chthamalus_combined, group.by = c('expl.var', 'algo', 'merged.by.run'))
plot <- val_importance_boxplot_merged$plot + theme_classic() + labs(fill = "Type")
ggsave(plot, file = paste0(figure_path, "VarImpBoxplot_ExplVar_Algo.png"), width = 8, height = 8)

# Response curves
models_to_plot <- get_built_models(myBiomodEM_chthamalus_combined)[c(1, 6, 7)]

# median
response_median <- bm_PlotResponseCurves(bm.out = myBiomodEM_chthamalus_combined, 
                      models.chosen = models_to_plot,
                      fixed.var = 'median')
tab <- response_median$tab
plot <- response_median$plot + theme_classic() + labs(colour = "Prediction Name")
...
# min
response_min<-bm_PlotResponseCurves(bm.out = myBiomodEM_chthamalus_combined, 
                      models.chosen = models_to_plot,
                      fixed.var = 'min')
tab <- response_min$tab
plot <- response_min$plot + theme_classic() + labs(colour = "Prediction Name")
...
# heat map - median
response_heatmap_median <- bm_PlotResponseCurves(bm.out = myBiomodEM_chthamalus_combined, 
...
current_projections_plot <- cowplot::plot_grid(
  ...
  chthamalus_combined_current_plot_TSS, 
  CM_diff_plot,
  ncol = 3, labels = c("(a) GBIF-only (SB)", "(b) GBIF-eDNA (SB)", "(c) Difference (SB)", "(d) GBIF-only (CM)", "(e) GBIF-eDNA (CM)", "(f) Difference (CM)"), align = "hv", label_fontface = "bold"
)
...
SB_row <- plot_grid(
  ...
  SB_diff_plot,
  ncol = 3,
  labels = c("a) GBIF-only", "b) GBIF-eDNA", "c) Difference"),
  align = "hv"
)
...
CM_row <- plot_grid(
  ...
  CM_diff_plot,
  ncol = 3,
  labels = c("d) GBIF-only", "e) GBIF-eDNA", "f) Difference"),
  align = "hv"
)
...
# visualise
future_semibalanus_visual <- get_predictions(myBiomodEMProjFuture_semibalanus_visual) # returns a rasterLayer
...
#FutureProj_semibalanus_visual <- get_predictions(myBiomodEMProjFuture_semibalanus_visual, metric.binary = "TSS")
...
# visualise
future_semibalanus_combined <- get_predictions(myBiomodEMProjFuture_semibalanus_combined) # returns a rasterLayer
...
# visualise
future_chthamalus_visual <- get_predictions(myBiomodEMProjFuture_chthamalus_visual) # returns a rasterLayer
...
# visualise
future_chthamalus_combined <- get_predictions(myBiomodEMProjFuture_chthamalus_combined) # returns a rasterLayer
...
eval.semi.bal.individual.visual <- read.csv("Data/Processed_Data/Semi.bal/Individual-Visual/Evaluations_All.csv", row.names = 1)
eval.semi.bal.individual.visual$model_type = "individual_algo"
eval.semi.bal.individual.visual$dataset = "GBIF-only"
eval.semi.bal.individual.visual$species = "Semibalanus balanoides"

eval.semi.bal.ensemble.visual <- read.csv("Data/Processed_Data/Semi.bal/Ensemble-Visual/Evaluations_All.csv", row.names = 1)
eval.semi.bal.ensemble.visual$model_type = "ensemble"
eval.semi.bal.ensemble.visual$dataset = "GBIF-only"
eval.semi.bal.ensemble.visual$species = "Semibalanus balanoides"

eval.semi.bal.individual.combined <- read.csv("Data/Processed_Data/Semi.bal/Individual-Combined/Evaluations_All.csv", row.names = 1)
...
eval.semi.bal.ensemble.combined <- read.csv("Data/Processed_Data/Semi.bal/Ensemble-Combined/Evaluations_All.csv", row.names = 1)
...
eval.chth.mon.individual.visual <- read.csv("Data/Processed_Data/Chtham.mont/Individual-Visual/Evaluations_All.csv", row.names = 1)
eval.chth.mon.individual.visual$model_type = "individual_algo"
eval.chth.mon.individual.visual$dataset = "GBIF-only"
eval.chth.mon.individual.visual$species = "Chthamalus montagui"

eval.chth.mon.ensemble.visual <- read.csv("Data/Processed_Data/Chtham.mont/Ensemble-Visual/Evaluations_All.csv", row.names = 1)
eval.chth.mon.ensemble.visual$model_type = "ensemble"
eval.chth.mon.ensemble.visual$dataset = "GBIF-only"
eval.chth.mon.ensemble.visual$species = "Chthamalus montagui"

eval.chth.mon.individual.combined <- read.csv("Data/Processed_Data/Chtham.mont/Individual-Combined/Evaluations_All.csv", row.names = 1)
...
eval.chth.mon.ensemble.combined <- read.csv("Data/Processed_Data/Chtham.mont/Ensemble-Combined/Evaluations_All.csv", row.names = 1)
...
mean_range_plot_mods_TSS <- ggplot(
  summary_eval %>%
    ...
    mutate(dataset = factor(
      dataset, levels = c("GBIF-only", "GBIF-eDNA")
    )),
  ...
  scale_colour_manual(
    values = c("GBIF-only" = GBIF_only_colour, 
               "GBIF-eDNA" = GBIF_eDNA_colour),
    breaks = c("GBIF-only", "GBIF-eDNA")
  ) +            # reorder datasets
  theme_bw() +
...
var.semi.bal.individual.visual <- read.csv("Data/Processed_Data/Semi.bal/Individual-Visual/VariablesImportance_All.csv", row.names = 1)
var.semi.bal.individual.visual$model_type = "individual_algo"
var.semi.bal.individual.visual$dataset = "GBIF-only"
var.semi.bal.individual.visual$species = "Semibalanus balanoides"

var.semi.bal.ensemble.visual <- read.csv("Data/Processed_Data/Semi.bal/Ensemble-Visual/VariablesImportance_All.csv", row.names = 1)
var.semi.bal.ensemble.visual$model_type = "ensemble"
var.semi.bal.ensemble.visual$dataset = "GBIF-only"
var.semi.bal.ensemble.visual$species = "Semibalanus balanoides"

var.semi.bal.individual.combined <- read.csv("Data/Processed_Data/Semi.bal/Individual-Combined/VariablesImportance_All.csv", row.names = 1)
...
var.semi.bal.ensemble.combined <- read.csv("Data/Processed_Data/Semi.bal/Ensemble-Combined/VariablesImportance_All.csv", row.names = 1)
...
var.chth.mon.individual.visual <- read.csv("Data/Processed_Data/Chtham.mont/Individual-Visual/VariablesImportance_All.csv", row.names = 1)
var.chth.mon.individual.visual$model_type = "individual_algo"
var.chth.mon.individual.visual$dataset = "GBIF-only"
var.chth.mon.individual.visual$species = "Chthamalus montagui"

var.chth.mon.ensemble.visual <- read.csv("Data/Processed_Data/Chtham.mont/Ensemble-Visual/VariablesImportance_All.csv", row.names = 1)
var.chth.mon.ensemble.visual$model_type = "ensemble"
var.chth.mon.ensemble.visual$dataset = "GBIF-only"
var.chth.mon.ensemble.visual$species = "Chthamalus montagui"

var.chth.mon.individual.combined <- read.csv("Data/Processed_Data/Chtham.mont/Individual-Combined/VariablesImportance_All.csv", row.names = 1)
...
var.chth.mon.ensemble.combined <- read.csv("Data/Processed_Data/Chtham.mont/Ensemble-Combined/VariablesImportance_All.csv", row.names = 1)
...
vars_boxplot_individual <- ggplot(
  ...
  labs(x = "Environmental variable", y = "Variable importance", fill = "Dataset") +
  scale_fill_manual(values = c("GBIF-only" = GBIF_only_colour, "GBIF-eDNA" = GBIF_eDNA_colour)) +
  scale_x_discrete(labels = c("SST", "pH", "Wave fetch")) +
  theme(strip.text = element_text(face = "italic"))
...
response_mean_SB_visual <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_semibalanus_visual, 
                      models.chosen = "all",
                      fixed.var = 'mean')
tab_mean_SB_visual <- response_mean_SB_visual$tab

response_mean_SB_visual_ensemble <- bm_PlotResponseCurves(bm.out = myBiomodEM_semibalanus_visual, 
                      models.chosen = "all",
                      fixed.var = 'mean')
...
# semibalanus - combined

response_mean_SB_combined <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_semibalanus_combined, 
                      models.chosen = "all",
                      fixed.var = 'mean')
tab_mean_SB_combined <- response_mean_SB_combined$tab

response_mean_SB_combined_ensemble <- bm_PlotResponseCurves(bm.out = myBiomodEM_semibalanus_combined, 
                      models.chosen = "all",
                      fixed.var = 'mean')
...
response_mean_CM_visual <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_chthamalus_visual, 
                      models.chosen = "all",
                      fixed.var = 'mean')
tab_mean_CM_visual <- response_mean_CM_visual$tab

response_mean_CM_visual_ensemble <- bm_PlotResponseCurves(bm.out = myBiomodEM_chthamalus_visual, 
                      models.chosen = "all",
                      fixed.var = 'mean')
...
response_mean_CM_combined <- bm_PlotResponseCurves(bm.out = myBiomodModelOut_chthamalus_combined, 
                      models.chosen = "all",
                      fixed.var = 'mean')
tab_mean_CM_combined <- response_mean_CM_combined$tab

response_mean_CM_combined_ensemble <- bm_PlotResponseCurves(bm.out = myBiomodEM_chthamalus_combined, 
                      models.chosen = "all",
                      fixed.var = 'mean')
