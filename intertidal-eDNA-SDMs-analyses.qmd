...
# Save the plot
ggsave(joint_map, filename = "Figures/chthamalus_semibalanus_map.png", width = 6, height = 4, dpi = 300)
```
...
# Rasterise points
rocky_rast <- rasterize(biotype_vect, rocky_template, field = 1, background = 0)

# Combine with existing rock raster
combined_rock_rast <- cover(rock_ukash_rast, rocky_rast)

# Plot
plot(combined_rock_rast, main = "Rocky substrate (binary)")
...
fetch_sf_proj <- st_transform(fetch_sf, crs(ocean_temp_present))
...
semibalanus_corrected_marclim <- get_nearest_coastline(
  enviro_vars_current,
...
semibalanus_corrected_marclim_PA <- get_nearest_coastline(
  enviro_vars_current,
...
# call function we created eariler to correct points
semibalanus_corrected_gbif <- get_nearest_coastline(
  enviro_vars_current,
...
table(na_points) #only 2 NA points
```
...
# call function we created eariler to correct points
semibalanus_corrected_combined <- get_nearest_coastline(
  enviro_vars_current,
...
# call function we created eariler to correct points
chthamalus_corrected_marclim <- get_nearest_coastline(
  enviro_vars_current,
...
chthamalus_corrected_marclim_PA <- get_nearest_coastline(
  enviro_vars_current,
...
# call function we created eariler to correct points
chthamalus_corrected_gbif <- get_nearest_coastline(
  enviro_vars_current,
...
table(na_points) #only 3 NA points
```
...
# call function we created eariler to correct points
chthamalus_corrected_combined <- get_nearest_coastline(
  enviro_vars_current,
...
### Finalise environmental stacks after tidying
...
For our models, the GBIF and eDNA data will be used to build the individual models, being split for instance 10 times into 70/30 percentages to cross-validate them (which is defined in the next section).
...
semibalanus_corrected_gbif_df <- semibalanus_corrected_gbif_df %>% select(-c("species"))

# Fix issues with non-matching coordinates
# Extract env values
env_vals <- extract(envir_terra, semibalanus_corrected_gbif)
df <- as.data.frame(geom(semibalanus_corrected_gbif))
df <- df %>% select(c(x, y))
colnames(df) <- c("lon", "lat")

# Bind back to coordinates
pts_with_vals <- cbind(df, env_vals)
...
myBiomodData_semibalanus_visual <- BIOMOD_FormatingData(
  ...
  PA.strategy = 'random',
  eval.resp.var = semibalanus_corrected_marclim_PA$Semibalanus.balanoides,
  # Evaluation responses (0/1)
  eval.resp.xy = semibalanus_corrected_marclim_PA[, c("lon", "lat")],
...
semibalanus_corrected_combined_df <- semibalanus_corrected_combined_df %>% select(-c("species"))

# Fix issues with non-matching coordinates
# Extract env values
env_vals <- extract(envir_terra, semibalanus_corrected_combined)
df <- as.data.frame(geom(semibalanus_corrected_combined))
df <- df %>% select(c(x, y))
colnames(df) <- c("lon", "lat")
...
chthamalus_corrected_gbif_df$chthamalus.balanoides <- 1 #add p for gbif
chthamalus_corrected_gbif_df <- chthamalus_corrected_gbif_df %>% select(-c("species"))

# Fix issues with non-matching coordinates
# Extract env values
env_vals <- extract(envir_terra, chthamalus_corrected_gbif)
df <- as.data.frame(geom(chthamalus_corrected_gbif))
df <- df %>% select(c(x, y))
colnames(df) <- c("lon", "lat")
...
myBiomodData_chthamalus_visual <- BIOMOD_FormatingData(
  ...
  # Number of pseudo-absences (~3 x number of presences)
  PA.strategy = 'random',
  eval.resp.var = chthamalus_corrected_marclim_PA$Chthamalus.montagui,
  # Evaluation responses (0/1)
  eval.resp.xy = chthamalus_corrected_marclim_PA[, c("lon", "lat")],
  # Evaluation locations,
...
chthamalus_corrected_combined_df$chthamalus.balanoides <- 1 #add p for gbif
chthamalus_corrected_combined_df <- chthamalus_corrected_combined_df %>% select(-c("species"))

# Fix issues with non-matching coordinates
# Extract env values
env_vals <- extract(envir_terra, chthamalus_corrected_combined)
df <- as.data.frame(geom(chthamalus_corrected_combined))
df <- df %>% select(c(x, y))
colnames(df) <- c("lon", "lat")
...
#just using weighted mean models from now on
...
plot(myBiomodEMProj_semibalanus_visual)
```
...
plot(myBiomodEMProj_semibalanus_combined)
```
...
plot(myBiomodEMProj_chthamalus_visual)
```
...
plot(myBiomodEMProj_chthamalus_combined)
```
